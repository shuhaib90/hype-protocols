/**
 * Pinata IPFS Uploader for Pixel Sentinels
 * Supports uploading metadata and preview images directly to Pinata IPFS.
 */
const https = require('https');
const fs = require('fs');
const path = require('path');

const PINATA_JWT = process.env.PINATA_JWT || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..88EIUIOeukVWBjXiOvTjZX0T3VD1dSYc7coIlrGsXR8';

const COLLECTION_DIR = path.join(
  process.env.USERPROFILE || 'C:\\Users\\shuhaib s',
  'OneDrive', 'Desktop', 'Documents', 'ChatGPT', 'New project', 'sentinel-10000'
);

async function pinJSONToIPFS(body, name) {
  return new Promise((resolve, reject) => {
    const postData = JSON.stringify({
      pinataOptions: { cidVersion: 1 },
      pinataMetadata: { name: name || 'PixelSentinel-Data' },
      pinataContent: body
    });

    const req = https.request('https://api.pinata.cloud/pinning/pinJSONToIPFS', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${PINATA_JWT}`
      }
    }, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
        try {
          const parsed = JSON.parse(data);
          resolve(parsed);
        } catch(e) {
          reject(new Error(data));
        }
      });
    });

    req.on('error', reject);
    req.write(postData);
    req.end();
  });
}

async function testConnection() {
  console.log('⚡ Checking Pinata IPFS connection...');
  return new Promise((resolve, reject) => {
    const req = https.request('https://api.pinata.cloud/data/testAuthentication', {
      headers: { 'Authorization': `Bearer ${PINATA_JWT}` }
    }, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
        console.log('Pinata Auth Result:', data);
        resolve(res.statusCode === 200);
      });
    });
    req.on('error', reject);
    req.end();
  });
}

// Upload Collection Storefront Metadata for OpenSea
async function uploadCollectionStorefront() {
  console.log('⚡ Uploading OpenSea Collection Storefront Metadata to Pinata IPFS...');
  const contractMetadata = {
    name: "Pixel Sentinels",
    description: "10,000 unique code-drawn robot portraits mined via in-browser GPU Proof-of-Work. Native 192x192 pixel art scaled to 1536x1536 nearest-neighbor sampling. Zero direct mints.",
    image: "https://hypeprotocols.xyz/sentinels/preview.png",
    external_link: "https://hypeprotocols.xyz",
    seller_fee_basis_points: 500, // 5% royalty
    fee_recipient: "0xb8E3DfDd19b6Bf35b9Fd87F8373F7f82C53bc93C"
  };

  const result = await pinJSONToIPFS(contractMetadata, 'PixelSentinels-ContractURI.json');
  console.log('✅ OpenSea Collection Metadata Pinned to IPFS:');
  console.log(`📌 IPFS CID: ipfs://${result.IpfsHash}`);
  console.log(`🌐 Gateway URL: https://gateway.pinata.cloud/ipfs/${result.IpfsHash}`);
  return result;
}

if (require.main === module) {
  (async () => {
    try {
      await testConnection();
      await uploadCollectionStorefront();
    } catch(err) {
      console.error('Error:', err);
    }
  })();
}

module.exports = {
  PINATA_JWT,
  COLLECTION_DIR,
  pinJSONToIPFS,
  testConnection,
  uploadCollectionStorefront
};
